...
```{r}
totalVax <- remDr$findElement(using='xpath','/html/body/app-bootstrap/ng2-bootstrap/bootstrap/div/div/div/div/div/div[1]/div[2]/div/div[1]/div[1]/div[1]/div/lego-report/lego-canvas-container/div/file-drop-zone/span/content-section/div[38]/canvas-component/div/div/div[1]/div/div/kpimetric/div/div[2]')
totalVax <- totalVax$getElementText()[[1]]
...
vaxVector <- c(dateUpdated_vax, totalVax)
vaxData <- rbind(vaxData, vaxVector)

vaxDataCleaned <- vaxData

vaxDataCleaned$date <- as.Date(vaxDataCleaned$date, format="%b %d, %Y")
vaxDataCleaned$totalVaccinated <- as.numeric(gsub(",","",vaxDataCleaned$totalVaccinated))
vaxDataCleaned <- vaxDataCleaned %>% 
  mutate(vaxDaily = totalVaccinated - lag(totalVaccinated))
```

```{r}
ggplot(vaxDataCleaned, aes(date, vaxDaily))+geom_col()+labs(title="People vaccinated per day\n(Mondays include weekend data)")
```

```{r}
vaxData <- vaxData[-c(5), ] # remove rows added during testing
vaxDataCleaned <- vaxDataCleaned[-c(5), ] # remove rows added during testing

```

```{r}
vaxData <- data.frame(date=c("Dec 17, 2020", "Dec 18, 2020", "Dec 21, 2020", "Dec 22, 2020", "Dec 23, 2020", "Dec 28, 2020", "Dec 29, 2020", "Dec 30, 2020"), totalVaccinated=c("0", "1,226","4,827","6,400","7,072","11,935", "12,869","17,020")) # initialize original df in case of issues
```
