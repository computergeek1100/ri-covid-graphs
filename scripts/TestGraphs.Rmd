...
```{r}
vaxSNE <- read.csv("https://raw.githubusercontent.com/youyanggu/covid19-cdc-vaccination-data/main/aggregated_adjusted.csv")%>%
...
vaxSNE$date <- as.Date(vaxSNE$date)
```
...
write.csv(vaxSNE, "../export/vax-percent-SNE-02-14-21.csv", row.names=FALSE)
```

```{r}
vaxPct <- ggplot(vaxSNE, aes(date, pct, color = state, group=1, text = paste0("Date: ", date,
                                                                      "\nState: ", state,
                                                                      "\n% Administered: ", pct)))+
  geom_line()+
  geom_line(data=vaxSNE[!is.na(vaxSNE$pct), ], linetype="dashed")+
...
```{r}
vaxRaw <- read.csv("https://raw.githubusercontent.com/youyanggu/covid19-cdc-vaccination-data/main/aggregated_adjusted.csv")%>%
  filter(Location == "RI")%>%
  select(date = Date, dose1 = Administered_Dose1, dose2 = Administered_Dose2)
```
```{r}
write.csv(vaxData_export, "../export/vaccine_data.csv", row.names = FALSE)
```

```{r}
vax_dist <- read.csv("https://raw.githubusercontent.com/youyanggu/covid19-cdc-vaccination-data/main/aggregated_adjusted.csv")%>%
  filter(Location == "RI")%>%
  select(date = Date, distributed = Doses_Distributed)

vax_dist$date <- as.Date(vax_dist$date) - 1

vax_dist <- left_join(vaxDataCleaned, vax_dist)%>%
  select(1, 5, 7)%>%
  mutate(percent = round(totalDoses / distributed * 100, 2))
```
```{r}
vaxPAD <- vaxDataCleaned%>%
  select(date, totalDose1)%>%
  padr::pad()

vaxPAD$totalDose1 <- na.locf(vaxPAD$totalDose1)
```

```{r}
vaccinations <- ggplot(vaxData_GRAPH, aes(x=date, y = number))+
  geom_col(position=position_stack(reverse=F), aes(fill=as.factor(dose), text = paste0("Date: ", date,
