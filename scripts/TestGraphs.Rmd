...
ggplot(vaxData2, aes(date, totalDoses))+
  ...
  labs(x="Date", y="Doses per Day (7-day average)")
```
```{r}
...
vaxRate_POLY <- lm(totalDoses~poly(numDaysD1,2, raw=TRUE), vaxData2)
vaxRate_POLY

orvaxRate_LOESS <- loess(avg7DayTotalDoses~numDays, vaxData2)
vaxRate_LOESS
```

$rate = 834.89 + 54.65*numDays$

$totalDoses = rate * numDays\\= 834.89 + 54.65(days)^2$
