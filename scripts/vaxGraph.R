...
if(all(vaxVector==as.character(tail(vaxData,1)))){ # Check if data in graph
  ...
  vaxDataCleaned$totalDose1 <- vaxDataCleaned$totalDose1 - vaxDataCleaned$totalDose2
  ...
  vaxGraph <- ggplot(vaxData_GRAPH, aes(date, number, fill=as.factor(dose), text = paste0("Date: ", date,
    ...
    geom_col(data=subset(vaxData_GRAPH, dose=="totalDose1" | dose=="totalDose2"), position=position_stack(reverse=F))+
    labs(title=paste0("Last Updated: ", format(tail(vaxDataCleaned$date, 1), "%b %d, %Y"),
                      "<sup>\nFirst Dose Only: ", formatC((tail(vaxDataCleaned$totalDose1, 1)), format = "d", big.mark = ","),
  ...
  vaxGraph$x$data[[1]]$name <- "First Dose"
